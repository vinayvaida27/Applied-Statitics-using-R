...
plot(wine, type = "l", col = "blue", ylab = "Wine Sales", xlab = "Time", main = "Australian Wine Sales")
lines(log(wine), col = "red", lty = 2)
legend("topleft", legend = c("Wine", "Log(Wine)"), col = c("blue", "red"), lty = c(1, 2))
...
lm_model <- lm(log(wine) ~ times  + Jan + Feb + Mar + Apr + May + Jun + Jul + Aug + Sep + Oct + Nov + Dec, data = data.frame(times,Jan, Feb, Mar, Apr, May, Jun, Jul, Aug, Sep, Oct, Nov, Dec, log(wine)))
...
lm_model_sin_cos <- lm(log(wine) ~ sint + cost + Jan, data = data.frame(times, sint, cost, Jan, log(wine)))
...
lines(df_fitted$times, df_fitted$arima_fitted, col = "green")
